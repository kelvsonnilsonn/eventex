...
public class EventController {
    ...
    @PostMapping
    public ResponseEntity<Long> create(@RequestBody CreateEventCommand command){
        return ResponseEntity.status(HttpStatus.CREATED).body(eventCommandService.create(command));
    }

    @DeleteMapping
    public ResponseEntity<Void> delete(@RequestBody DeleteEventCommand command){
        eventCommandService.delete(command);
        return ResponseEntity.status(HttpStatus.OK).build();
    }

    @PutMapping(AppConstants.CHANGE_EVENT_NAME)
    public ResponseEntity<Void> updateName(@RequestBody UpdateEventNameCommand command) {
        eventCommandService.update(command);
        return ResponseEntity.ok().build();
    }

    @PutMapping(AppConstants.CHANGE_DESCRIPTION_NAME)
    public ResponseEntity<Void> updateDescription(@RequestBody UpdateEventDescriptionCommand command) {
        eventCommandService.update(command);
        return ResponseEntity.ok().build();
    }
